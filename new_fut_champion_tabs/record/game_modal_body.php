					...
					$squad_players = array();
					$sql = 'SELECT * FROM squad AS S LEFT JOIN players AS P ON S.player_id=P.id LEFT JOIN (SELECT player_id, COUNT(*) AS app FROM appearances WHERE fut_champion_id='.$fut_champion_id.' AND game='.$game.' GROUP BY player_id) AS APP ON P.id=APP.player_id ORDER BY S.position ASC';
					$result = $conn->query($sql);
					while ($row = $result->fetch_assoc()) {
						if ($row["app"]!="" || $row["position"]<=11) $display = "block";
						else $display = "none";
						array_push($squad_players, array("id"=>$row["id"], "C_name"=>$row["C_name"], "display"=>$display));
					}
					...
					while ($row = $result->fetch_assoc()) {
						...
						foreach ($squad_players as $squad_player) {
							echo '
										<div class="dropdown-option player_'.$squad_player["id"].'" style="display: '.$squad_player["display"].';" onclick="choose_player(this, '.$squad_player["id"].', \''.$squad_player["C_name"].'\')">
					...
					while ($row = $result->fetch_assoc()) {
						...
						echo '
								<div class="col-xl-40 col-md-60" style="margin-bottom: 10px;" draggable="'.(($row["position"]<=11)?'true':'false').'" ondragstart="drag('.$row["player_id"].', \''.$row["C_name"].'\')">
									<div class="row" style="height: 120px;">
										...
										<div style="height: 100%;" class="col-70">
											<input name="appearances[]" value="'.$row["player_id"].'" type="checkbox"'.$checked.' onclick="sub_on(this, '.$row["player_id"].')">
											&nbsp;
