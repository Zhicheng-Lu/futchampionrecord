				...
				// win
				$sql = 'SELECT COUNT(*) AS win FROM results AS R WHERE (score1>score2 OR penalty1>penalty2)'.$search_citeria;
				$result = $conn->query($sql);
				while ($row = $result->fetch_assoc()) {
					$win = $row["win"];
				}
				// loss
				$sql = 'SELECT COUNT(*) AS loss FROM results AS R WHERE (score1<score2 OR penalty1<penalty2)'.$search_citeria;
				$result = $conn->query($sql);
				while ($row = $result->fetch_assoc()) {
					$loss = $row["loss"];
				}
				// goal & conceed
				if ($game_player_name == "") $sql = 'SELECT SUM(score1) AS score, SUM(score2) AS conceed FROM results AS R';
				else $sql = 'SELECT SUM(score1) AS score, SUM(score2) AS conceed FROM results AS R WHERE R.game_player="'.$game_player_name.'"';
				$result = $conn->query($sql);
				while ($row = $result->fetch_assoc()) {
					$score = $row["score"];
					$conceed = $row["conceed"];
				...
				if ($game_player_name == "") {
					$sql = 'SELECT APP.player_id, PLA.E_name, PLA.C_name, PLA.rating, APP.appearance, SCO.num_score, ASS.num_assist FROM (SELECT player_id, COUNT(*) AS appearance FROM appearances GROUP BY player_id) AS APP LEFT JOIN (SELECT scorer, COUNT(*) AS num_score FROM goals GROUP BY scorer) AS SCO ON APP.player_id=SCO.scorer LEFT JOIN (SELECT assist, COUNT(*) AS num_assist FROM goals GROUP BY assist) AS ASS ON APP.player_id=ASS.assist LEFT JOIN players AS PLA on APP.player_id=PLA.id ORDER BY '.$order_by;
				}
				else {
					$sql = 'SELECT APP.player_id, PLA.E_name, PLA.C_name, PLA.rating, APP.appearance, SCO.num_score, ASS.num_assist FROM (SELECT APP.player_id, COUNT(*) AS appearance FROM appearances AS APP LEFT JOIN results AS R ON APP.fut_champion_id=R.fut_champion_id AND APP.game=R.game WHERE R.game_player="'.$game_player_name.'" GROUP BY APP.player_id) AS APP LEFT JOIN (SELECT SCO.scorer, COUNT(*) AS num_score FROM goals AS SCO LEFT JOIN results AS R ON SCO.fut_champion_id=R.fut_champion_id AND SCO.game=R.game WHERE R.game_player="'.$game_player_name.'" GROUP BY SCO.scorer) AS SCO ON APP.player_id=SCO.scorer LEFT JOIN (SELECT ASS.assist, COUNT(*) AS num_assist FROM goals AS ASS LEFT JOIN results AS R ON ASS.fut_champion_id=R.fut_champion_id AND ASS.game=R.game WHERE R.game_player="'.$game_player_name.'" GROUP BY assist) AS ASS ON APP.player_id=ASS.assist LEFT JOIN players AS PLA on APP.player_id=PLA.id ORDER BY '.$order_by;
				}
